...
class Page extends React.Component {
  ...
  async componentDidMount() {
    ...
    this.fields = this.table.length > 0 ? Object.keys(this.table[0]) : []
  }
  ...
  render() {
    return this.state.isLoaded && <div onKeyDown={this.onKeyPressed}>
      ...
      {/*  {user => (<Token name={user} pass={'changeme'}/>)}*/}
      {this.Layout()}
      {/*</UserContext.Consumer>*/}
  ...
  Sidebar() {
    return <input type={'password'} name={'password'} onChange={this.setPassword}/>
  }
  ...
  Token(username) {
    return <button onClick={() => this.getToken(username)}>{username}</button>
  }
